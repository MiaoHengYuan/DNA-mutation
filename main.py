...
def get_results(df_data, cancer_type, k=0, j=50):
    print('>>>>>>>>> code start!')
...
if __name__ == "__main__":
    ...
    type_list = ['Breast','Lung Squamous', 'Prostate', 'Pilocytic Astrocytoma','Neuroblastoma','Melanoma','Medulloblastoma',\
                'Lung Adeno','ALL','Lymphoma B-cell','Colorectum','Stomach','Esophageal','Head and Neck','AML',\
                'Bladder','Glioma Low Grade','Kidney Clear Cell','Uterus','Kidney Papillary','Glioblastoma','Myeloma',\
                'Thyroid','Kidney Chromophobe','Pancreas','Cervix','Liver','Ovary','CLL','Lung Small Cell']

    for i in type_list:
        print('>>>>>>>>>Hello,the code access to type '+i+'>>>>>>>>>')
        df_data = pd.read_csv('/home/lthpc/MHY/DNA_mutation/somatic_mutation_data/'+i+'/'+i+'_raw_mutations_data.txt', \
                                delimiter="\t", header=None)

        get_results(df_data, i, 0, 50)