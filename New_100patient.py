...
def get_results(df_breast, cancer_type, k=0, j=50):
    ...
    if cancer_type == 'Lung Squamous':
        patient_list = df_breast[0].value_counts()[k+22:j].index.tolist()
    else:
    ...
    for patient_id in patient_list:
        ...
        if not os.path.exists('Correct_' + cancer_type + '/new_{}'.format(patient_id)):
            os.makedirs('Correct_' + cancer_type + '/new_{}'.format(patient_id))
        ...
        for c in distance.keys():
            ...
            dic_chr['chr' + c], dic_chr_new['chr' + c] = fit_function(df_bucket[filter_col], distance[c], c, False, False, threshold_list[c])
        ...
        for c in distance.keys():
            ...
            se_new, sp_new, le_new, lp_new = parameter_conclude_1(dic_chr_new['chr' + c])

            parameter_conclude_2(se_, sp_, le_, lp_, patient_id, c, path, False, False, plot_withDistance=True)
            parameter_conclude_2(se_new, sp_new, le_new, lp_new, patient_id, c, path_3, False, False, plot_withDistance=True)

            D['dict_chr' + c]['shrot_exp'] = se_
            D['dict_chr' + c]['shrot_pow'] = sp_
            D['dict_chr' + c]['long_exp'] = le_
            D['dict_chr' + c]['long_pow'] = lp_
            l['dict_chr_new' + c]['shrot_exp'] = se_new
            l['dict_chr_new' + c]['shrot_pow'] = sp_new
            l['dict_chr_new' + c]['long_exp'] = le_new
            l['dict_chr_new' + c]['long_pow'] = lp_new
        ...
        for c in distance.keys():
            ...
            try:
                ...
                se_new = l['dict_chr_new' + c]['shrot_exp']
                plt.plot(list(zip(*se_new))[0], -1 * np.array(list(zip(*se_new))[1]))
                plt.scatter(list(zip(*se_new))[0], -1 * np.array(list(zip(*se_new))[1]), color='palevioletred')
                plt.title("short_expon_{}".format(patient_id))
                plt.savefig(path_3+'/shot_expon_{}.jpg'.format(c))
                plt.close()
            ...
            try:
                ...
                sp_new = l['dict_chr_new' + c]['shrot_pow']
                plt.plot(list(zip(*sp_new))[0], -1 * np.array(list(zip(*sp_new))[1]))
                plt.scatter(list(zip(*sp_new))[0], -1 * np.array(list(zip(*sp_new))[1]), color='palevioletred')
                plt.title("short_powerlaw_{}".format(patient_id))
                plt.savefig(path_3+'/shot_powerlaw_{}.jpg'.format(c))
                plt.close()
            ...
            try:
                ...
                le_new = l['dict_chr_new' + c]['long_exp']
                plt.plot(list(zip(*le_new))[0], -1 * np.array(list(zip(*le_new))[1]))
                plt.scatter(list(zip(*le_new))[0], -1 * np.array(list(zip(*le_new))[1]), color='palevioletred')
                plt.title("long_expon_{}".format(patient_id))
                plt.savefig(path_3+'/long_expon_{}.jpg'.format(c))
                plt.close()
            except IndexError:
                pass
            ...
            try:
                ...
                lp_new = l['dict_chr_new' + c]['long_pow']
                plt.plot(list(zip(*lp_new))[0], -1 * np.array(list(zip(*lp_new))[1]))
                plt.scatter(list(zip(*lp_new))[0], -1 * np.array(list(zip(*lp_new))[1]), color='palevioletred')
                plt.title("long_powerlaw_{}".format(patient_id))
                plt.savefig(path_3+'/long_powerlaw_{}.jpg'.format(c))
                plt.close()
            except IndexError:
                pass
        ...
        PLVD_new[patient_id] = dic_chr_new
    f = open('/home/lthpc/MHY/New_DNA/ParaDct/file_{}.pkl'.format(cancer_type), 'wb')
    pickle.dump(PLVD, f)
    f.close()

    f = open('/home/lthpc/MHY/New_DNA/New_ParaDct/file_new_{}.pkl'.format(cancer_type), 'wb')
    pickle.dump(PLVD_new, f)
    f.close()


if __name__ == "__main__":

    #type_list = ['Medulloblastoma','Pancreas','Prostate','Stomach','Uterus','Colorectum','Lung Squamous','Lymphoma B-cell','Melanoma',"Bladder","CLL","Glioblastoma","Head and Neck","Liver","Lung Adeno"]
    #type_list = ['Ovary','Pancreas','Pilocytic Astrocytoma','Prostate','Stomach','Thyroid','Uterus']
    #type_list = ['Breast']
    #type_list = ['Colorectum','Esophageal','Lung Squamous','Lymphoma B-cell','Melanoma']
    #type_list = ['Lung Squamous','Prostate','Pilocytic Astrocytoma','Neuroblastoma','Melanoma','Medulloblastoma','Lung Adeno','ALL','Lymphoma B-cell','Colorectum','Stomach','Esophageal','Head and Neck','AML',\
    #        'Glioma Low Grade','Bladder','Kidney Clear Cell','Uterus','Kidney Papillary','Glioblastoma','Myeloma','Thyroid','Kidney Chromophobe','Pancreas','Cervix','Liver','Ovary','CLL','Lung Small Cell']
    #type_list = ['Kidney Clear Cell','Uterus','Kidney Papillary','Glioblastoma','Myeloma','Thyroid','Kidney Chromophobe','Pancreas','Cervix','Liver','Ovary','CLL','Lung Small Cell']
    type_list = ['Lung Squamous', 'Prostate', 'Pilocytic Astrocytoma','Neuroblastoma','Melanoma','Medulloblastoma','Lung Adeno','ALL','Lymphoma B-cell','Colorectum','Stomach',\
     'Esophageal','Head and Neck','AML','Bladder','Glioma Low Grade','Kidney Clear Cell','Uterus','Kidney Papillary','Glioblastoma','Myeloma','Thyroid','Kidney Chromophobe',\
     'Pancreas','Cervix','Liver','Ovary','CLL','Lung Small Cell']

    for i in type_list:
        print('>>>>>>>>>Hello,the code access to type '+i+'>>>>>>>>>')
        df_breast = pd.read_csv('/home/lthpc/MHY/DNA_mutation/somatic_mutation_data/'+i+'/'+i+'_raw_mutations_data.txt', \
                                delimiter="\t", header=None)

        get_results(df_breast, i, 0, 50)