...
def fit_function(df, distance,c, visualization_1=False, visualization_2=False, threshold_list=[]):
 ...
 for i in range(sliding_time):
  ...
  if len(y_raw)==0:
   ...
   if fit_short.distribution_compare('power_law', 'exponential')[0]>=0:
    indicator = 1
   else:
    indicator = 0
  ...
  if indicator == 1: ### power-law
   if len(y_) <=1:
    ...
    short_dict['powerlaw'] = (params_new, np.nan)
  ...
  if len(y_rawl)==0:
   ...
   if fit_long.distribution_compare('power_law', 'exponential')[0]>=0:
    indicator = 1
   else:
    indicator = 0
  ...
  if indicator == 1:  ### powerlaw
   if len(y_l) <=1:
    ...
    long_dict['powerlaw'] = (params_new, np.nan)
